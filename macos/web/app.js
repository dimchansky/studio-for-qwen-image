...
let session=null,sessions=[],attachments=[],job=null,lastStatus=null,polling=false,pageName='workspace',submitting=false,pendingJobs=[];
...
function updateSend(){$('#send').disabled=!$('#prompt').value.trim()||submitting;const label=submitting?'正在发送':job?'加入队列':'生成图片';$('#send').setAttribute('aria-label',label);$('#send').title=label}
...
$('#composer').onsubmit=safe(async e=>{
 ...
 try{
  ...
  const created=await api('generate',{session_id:sid,prompt,images:attachments,...generationOptions()});
...
document.addEventListener('DOMContentLoaded',()=>{Promise.all([restorePreferences(),refreshSessions(),refreshStatus()]).then(()=>scheduleEstimate()).catch(e=>toast('连接失败：'+e.message));setInterval(()=>refreshStatus().catch(()=>{}),2000);setInterval(pollJob,800);setMode();updateSend()},{once:true});
