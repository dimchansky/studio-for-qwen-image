...
class TranslationCoverage(unittest.TestCase):
    def test_all_static_markup(self):
        for platform in ('macos','windows'):
            web=ROOT/platform/'web';catalog=json.loads((web/'locales.json').read_text())
            for filename in ('index.html','environment.html'):
                parsed=Markup();parsed.feed((web/filename).read_text())
    ...
    def test_backend_application_copy(self):
        ...
        for platform in ('macos','windows'):
            catalog=json.loads((ROOT/platform/'web/locales.json').read_text())
            missing=set()
