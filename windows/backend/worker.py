...
try:
    ...
    kwargs=dict(prompt=prompt,width=p['width'],height=p['height'],num_inference_steps=p['steps'],generator=torch.Generator('cuda').manual_seed(seed),callback_on_step_end=step,output_resolution=max(p['width'],p['height']))
    if refs: kwargs['image']=refs if len(refs)>1 else refs[0]
    result=pipe(**kwargs).images[0]
    report(stage='正在保存图片',progress=.98)
    from PIL.PngImagePlugin import PngInfo
    metadata=PngInfo();metadata.add_text('qwen_studio',json.dumps({**{k:p.get(k) for k in ('prompt','width','height','steps','transparent')},'seed':seed,'decoder':'full-frame-fp32'},ensure_ascii=False))
    name=p['job_id']+'.png';result.save(Path(p['data'])/'images'/name,pnginfo=metadata)
