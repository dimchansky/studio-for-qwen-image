"""Selectable official sources, parallel range downloads, resumable and SHA-256 checked."""
...
def chunk_download(task):
 ...
 for attempt in range(6):
  ...
  result=subprocess.run([shutil.which('curl.exe') or shutil.which('curl') or 'curl','--fail','--location','--connect-timeout','20','--max-time','900','--range',f'{start+offset}-{end}','--output',str(tmp),'--silent','--show-error',url(item)],creationflags=getattr(subprocess,'CREATE_NO_WINDOW',0))
