...
$('#composer').onsubmit=safe(async e=>{
 ...
 if(mode==='image'&&$('#enhance').checked&&!lastStatus?.enhancers?.[attachments.length?'pe-i2i':'pe-t2i']?.ready){showPage('settings');return toast('请先下载本次任务所需的提示词增强模型。')}
 ...
 try{
  ...
  const created=await api('generate',{session_id:sid,prompt,mode,images:attachments,width,height,...generationOptions(),steps:Number($('#steps').value),seed:Number($('#seed').value),transparent:$('#transparent').checked,chat_model:selectedModel,think:thinking});
...
$('#download').onclick=safe(async()=>{downloadTarget='image';if(!lastStatus?.model.source){chooseDownloadSource();return}await beginDownload(lastStatus.model.source)});
