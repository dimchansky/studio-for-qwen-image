...
async function api(path,body){const r=await fetch('/api/'+path,{method:body===undefined?'GET':'POST',headers:{'Content-Type':'application/json','X-Studio-Token':token},body:body===undefined?undefined:JSON.stringify(body)});const data=await r.json();if(!r.ok)throw Error(data.error||'请求失败');return data}
...
function updateModelControls(){
 $('#selected-model').textContent=selectedModel||'选择模型';
...
function renderSession(){const before=$('#composer-area').getBoundingClientRect();const wasEmpty=$('#workspace').classList.contains('empty');const empty=!session?.messages.length;$('#workspace').classList.toggle('empty',empty);$('#welcome').hidden=!empty;syncWelcome();$('#thread-title').textContent=session?.title||'新会话';$('#session-menu').hidden=!session;const list=$('#messages');list.replaceChildren();(session?.messages||[]).forEach(m=>{const el=document.createElement('article');el.className='message '+m.role+(m.meta.error?' error':'');const role=document.createElement('div');role.className='role';role.textContent=m.role==='user'?'你':m.meta.mode==='chat'?(m.meta.model||'Ollama'):'Qwen Studio';el.append(role);if(m.content){const c=document.createElement('div');c.className='content';c.textContent=m.content;el.append(c)}if(m.images.length){const imgs=document.createElement('div');imgs.className='message-images';m.images.forEach(name=>imgs.append(imageElement(name)));el.append(imgs);if(m.role==='assistant'){el.append(imageButtons(m.images[0]));const meta=document.createElement('div');meta.className='image-meta';meta.textContent=`${m.meta.width} × ${m.meta.height}　${m.meta.steps} 步　${m.meta.seconds} 秒`;el.append(meta)}}list.append(el)});messagesFollowEnd=true;renderJob();renderTaskTray();scrollMessagesToEnd();if(wasEmpty!==empty&&pageName==='workspace'){const after=$('#composer-area').getBoundingClientRect();motion($('#composer-area'),[{transform:`translate(${before.left-after.left}px,${before.top-after.top}px)`},{transform:'translate(0,0)'}],420);motion(list,[{opacity:0},{opacity:1}],300)}}
...
$('#composer').onsubmit=safe(async e=>{
 ...
 try{
  ...
  const created=await api('generate',{session_id:sid,prompt,mode,images:attachments,width,height,steps:Number($('#steps').value),seed:Number($('#seed').value),transparent:$('#transparent').checked,chat_model:selectedModel,think:thinking});
...
$('#close-settings').onclick=()=>showPage('workspace');$('#options-button').onclick=()=>{const d=$('#options');d.showModal();motion(d,[{opacity:0,transform:'translateY(8px) scale(.98)'},{opacity:1,transform:'translateY(0) scale(1)'}])};$('#apply-options').onclick=()=>{if(!$('#steps').checkValidity()||!$('#seed').checkValidity())return toast('请检查步数和种子的取值范围');$('#options-label').textContent=$('#size').value.split(',').join(' × ');$('#options').close()};
...
function renderAttachments(){$('#attachments').replaceChildren();attachments.forEach(name=>{const el=document.createElement('div');el.className='attachment';el.append(imageElement(name));const b=document.createElement('button');b.type='button';b.setAttribute('aria-label','移除参考图');b.append(icon('close'));b.onclick=()=>{attachments=attachments.filter(x=>x!==name);renderAttachments()};el.append(b);$('#attachments').append(el)})}
...
$('#delete-session').onclick=safe(async()=>{if(!confirm('确定删除这个会话及其聊天记录？'))return;await api('session/delete',{id:session.id});$('#session-options').close();reset();await refreshSessions()});
...
function renderDownloadSources(model){
 ...
 select.disabled=model.downloading||downloadStarting||sourceSaving;
...
async function beginDownload(source){
 ...
 try{await api('model/download',{source});$('#download-source-dialog').close();await refreshStatus()}
 finally{downloadStarting=false;$('#download').disabled=!!(lastStatus?.model.ready||lastStatus?.model.downloading);$('#confirm-download').disabled=!$('#download-source-options input:checked');$('#download-source').disabled=!!lastStatus?.model.downloading}
}
$('#download').onclick=safe(async()=>{if(!lastStatus?.model.source){chooseDownloadSource();return}await beginDownload(lastStatus.model.source)});
...
async function refreshStatus(){lastStatus=await api('status');const m=lastStatus.model;renderDownloadSources(m);const ratio=m.bytes/m.total;const label=m.ready?'图像模型已就绪':m.verifying?'正在校验模型':m.bytes>0?`模型下载 ${Math.floor(ratio*100)}%`:'图像模型待下载';$('#model-status span:last-child').textContent=label;$('#model-status .status-dot').classList.toggle('amber',!m.ready);$('#download-info').textContent=m.ready?'已就绪':`${(m.bytes/1e9).toFixed(1)} / ${(m.total/1e9).toFixed(1)} GB`;$('#download-progress').value=ratio;const speed=m.speed||0;$('#download-speed').textContent=m.ready?'下载完成':m.verifying?'正在校验文件':speed>0?`${speed>=1e6?(speed/1e6).toFixed(1)+' MB/s':(speed/1e3).toFixed(0)+' KB/s'}`:m.downloading?'正在测量速度…':m.bytes>0?'下载已暂停':'等待开始下载';const eta=m.eta;$('#download-eta').textContent=m.ready?'可以开始创作':m.verifying?'即将完成':eta?`预计剩余 ${eta>=3600?Math.floor(eta/3600)+' 小时 ':''}${Math.ceil((eta%3600)/60)} 分钟`:'剩余时间待估算';$('#download').disabled=m.ready||m.downloading||downloadStarting;$('#download').textContent=m.ready?'已下载':m.downloading?'正在下载…':m.bytes>0?'继续下载':'下载模型';$('#download-error').hidden=!m.error;$('#download-error').textContent=m.error;$('#ollama-state').textContent=lastStatus.ollama?'已连接':'未连接，请启动 Ollama';$('#data-path').textContent=lastStatus.data;const names=lastStatus.chat_models;if(!names.includes(selectedModel))selectedModel=names.includes('gpt-oss:20b')?'gpt-oss:20b':names[0]||'';updateModelControls();pendingJobs=lastStatus.pending||[];if(lastStatus.active?.state==='running'&&!job){job=lastStatus.active;renderJob()}renderTaskTray();updateSend()}
...
let welcomeLanguage='zh',welcomeCycle=true,welcomeIndex=0,welcomeKey='',welcomeTimer=null,welcomeVersion=0;
$('#welcome-language').value=welcomeLanguage;$('#welcome-cycle').checked=welcomeCycle;
function welcomeLocale(){return welcomeLanguage==='auto'?(navigator.language.startsWith('zh')?'zh':'en'):welcomeLanguage}
...
$('#welcome-language').onchange=safe(async e=>{welcomeLanguage=e.target.value;syncWelcome(true);await api('preferences',{welcome_language:welcomeLanguage})});
...
async function restorePreferences(){const p=await api('preferences');welcomeLanguage=p.welcome_language;welcomeCycle=p.welcome_cycle;$('#welcome-language').value=welcomeLanguage;$('#welcome-cycle').checked=welcomeCycle;syncWelcome(true)}
Promise.all([restorePreferences(),refreshSessions(),refreshStatus()]).catch(e=>toast('连接失败：'+e.message));setInterval(()=>refreshStatus().catch(()=>{}),2000);setInterval(pollJob,800);setMode('image');updateSend();
